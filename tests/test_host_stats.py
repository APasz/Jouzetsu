...
def test_host_stat_meter_rules_cover_bounded_activity_values() -> None:
    ...
    assert ".jouzetsu-host-stat-meter-0 { width: 0%; background: linear-gradient(90deg, #123456, #123456); }" in rules
    assert ".jouzetsu-host-stat-meter-100 { width: 100%; background: linear-gradient(90deg, #123456, #fedcba); }" in rules
...
def test_host_stats_retains_a_rolling_cpu_average() -> None:
    ...
    assert snapshot.average_cpu_utilization_percent is None or 0.0 <= snapshot.average_cpu_utilization_percent <= 100.0


def test_host_stats_reuses_slow_metrics_between_fast_cpu_samples(monkeypatch: MonkeyPatch) -> None:
...
def test_integrated_gpu_is_hidden_unless_explicitly_enabled() -> None:
    ...
    assert is_gpu_enabled(integrated_gpu, {integrated_gpu.id: HostStatsDeviceSettings(visible=True)})
    assert not is_gpu_enabled(integrated_gpu, {integrated_gpu.id: HostStatsDeviceSettings(visible=False)})
