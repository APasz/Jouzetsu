...
class _Decoder:
    ...
    def optional_integer(self, raw: object, *, path: str, default: int | None) -> int | None:
        if raw is None:
            return None
        return self.integer(raw, path=path, default=default if default is not None else 1)
...
def decode_config(raw: object, *, paths: AppPaths) -> AppConfig:
    ...
    version: int = decoder.integer(root.get("version"), path="config.version", default=CONFIG_VERSION)
    ...
    config: AppConfig = AppConfig(
        ...
        generation=_decode_generation(decoder, root.get("generation"), defaults.generation),
        ...
        host_stats=_decode_host_stats(decoder, root.get("host_stats"), defaults.host_stats),
...
def encode_config(config: AppConfig) -> ConfigDocument:
    ...
    document: ConfigDocument = {
        ...
        "logging": {
            ...
            "directory": _path_to_config_string(config.log_directory, paths=config.paths),
        },
        "host_stats": {
...
def _decode_server(decoder: _Decoder, raw: object, defaults: ServerSettings) -> ServerSettings:
    values: dict[str, object] = decoder.object(
        raw,
        path="config.server",
        required=frozenset({"base_url", "api_key", "default_model", "model_aliases", "auto_unload_minutes"}),
    )
    return ServerSettings(
        base_url=decoder.string(values.get("base_url"), path="config.server.base_url", default=defaults.base_url),
        api_key=decoder.string(values.get("api_key"), path="config.server.api_key", default=defaults.api_key),
        default_model=decoder.string(
            values.get("default_model"), path="config.server.default_model", default=defaults.default_model
        ),
...
def _decode_generation(
    ...
    values: dict[str, object] = decoder.object(
        ...
        required=frozenset(
            {"temperature", "top_p", "max_tokens", "system_prompt", "continuity_review", "british_english"}
        ),
    ...
    return GenerationSettings(
        temperature=decoder.number(values.get("temperature"), path="config.generation.temperature", default=defaults.temperature),
        top_p=decoder.number(values.get("top_p"), path="config.generation.top_p", default=defaults.top_p),
        max_tokens=decoder.integer(values.get("max_tokens"), path="config.generation.max_tokens", default=defaults.max_tokens),
        system_prompt=decoder.string(
            values.get("system_prompt"), path="config.generation.system_prompt", default=defaults.system_prompt
        ),
...
def _decode_ui(decoder: _Decoder, raw: object, defaults: UiSettings) -> UiSettings:
    ...
    return UiSettings(
        host=decoder.string(values.get("host"), path="config.ui.host", default=defaults.host),
        port=decoder.integer(values.get("port"), path="config.ui.port", default=defaults.port),
        dark_mode=decoder.boolean(values.get("dark_mode"), path="config.ui.dark_mode", default=defaults.dark_mode),
        ...
        active_chat_id=decoder.string(
            values.get("active_chat_id"), path="config.ui.active_chat_id", default=defaults.active_chat_id
        ),
...
def _decode_theme(decoder: _Decoder, raw: object, defaults: ThemeSettings) -> ThemeSettings:
    if raw is None:
        return defaults
...
def _decode_logging(
    ...
    directory_raw: str = decoder.string(
        values.get("directory"), path="config.logging.directory", default=str(paths.log_directory)
    )
    directory: Path = Path(directory_raw).expanduser()
    if not directory.is_absolute():
        directory = paths.home / directory
    return LoggingSettings(
        enabled=decoder.boolean(values.get("enabled"), path="config.logging.enabled", default=defaults.enabled),
        directory=directory,
    )
...
def _decode_host_stats(
    ...
    return HostStatsSettings(
        system=decoder.boolean(values.get("system"), path="config.host_stats.system", default=defaults.system),
        cpu=decoder.boolean(values.get("cpu"), path="config.host_stats.cpu", default=defaults.cpu),
        gpu=decoder.boolean(values.get("gpu"), path="config.host_stats.gpu", default=defaults.gpu),
        network=decoder.boolean(values.get("network"), path="config.host_stats.network", default=defaults.network),
        gpus=_decode_host_devices(decoder, values.get("gpus"), path="config.host_stats.gpus"),
...
def _decode_access(decoder: _Decoder, raw: object, defaults: AccessSettings) -> AccessSettings:
    values: dict[str, object] = decoder.object(
        raw,
        path="config.access",
        required=frozenset(
    ...
    devices_raw: dict[str, object] = decoder.mapping(values.get("devices"), path="config.access.devices")
    devices: dict[str, DeviceAccessSettings] = {}
    required_device_fields: frozenset[str] = frozenset(
        {"access_allowed", "label", "last_ip", "hostname", "first_seen_at", "last_seen_at"}
    )
    for device_id, device_raw in devices_raw.items():
        ...
        devices[device_id] = DeviceAccessSettings(
            ...
            label=decoder.string(device_values.get("label"), path=f"config.access.devices.{device_id}.label", default=""),
            last_ip=decoder.string(device_values.get("last_ip"), path=f"config.access.devices.{device_id}.last_ip", default=""),
            hostname=decoder.string(device_values.get("hostname"), path=f"config.access.devices.{device_id}.hostname", default=""),
            first_seen_at=decoder.string(device_values.get("first_seen_at"), path=f"config.access.devices.{device_id}.first_seen_at", default=""),
            last_seen_at=decoder.string(device_values.get("last_seen_at"), path=f"config.access.devices.{device_id}.last_seen_at", default=""),
        )
    return AccessSettings(
        default_private=decoder.boolean(
            values.get("default_private"), path="config.access.default_private", default=defaults.default_private
        ),
        ...
        approval_phrase=decoder.string(
            values.get("approval_phrase"), path="config.access.approval_phrase", default=defaults.approval_phrase
        ),
        devices=devices,
    )


def _decode_string_mapping(decoder: _Decoder, raw: object, *, path: str) -> dict[str, str]:
...
def _decode_replacements(decoder: _Decoder, raw: object, *, path: str) -> list[SpellingReplacement]:
    replacements: list[SpellingReplacement] = []
    for index, item in enumerate(decoder.list(raw, path=path)):
        ...
        replacements.append(
            SpellingReplacement(
                source=decoder.string(values.get("source"), path=f"{path}[{index}].source", default=""),
                replacement=decoder.string(
                    values.get("replacement"), path=f"{path}[{index}].replacement", default=""
                ),
            )
        )
    return replacements
        ...
        prompts.append(
            StarterPrompt(
                label=decoder.string(
                    values.get("label"), path=f"config.ui.starter_prompts[{index}].label", default=""
                ),
                content=decoder.string(
                    values.get("content"), path=f"config.ui.starter_prompts[{index}].content", default=""
                ),
            )
        )
...
def _decode_host_devices(
    ...
    for device_id, device_raw in raw_values.items():
        ...
        devices[device_id] = HostStatsDeviceSettings(
            visible=decoder.boolean(values.get("visible"), path=f"{path}.{device_id}.visible", default=True),
            label=decoder.string(values.get("label"), path=f"{path}.{device_id}.label", default=""),
        )
    return devices
...
def _decode_hex(decoder: _Decoder, raw: object, *, path: str, default: str) -> str:
    ...
    if not value.startswith("#") or len(value) != 7 or any(character not in "0123456789abcdefABCDEF" for character in value[1:]):
